...
export class ObserverEngine {
  ...
  processEvent(event) {
    ...
    // Only process tool_use events for violation detection
    if (event.type === "tool_use" || event.type === "tool_result") {
      ...
      if (isWrite) {
        this._writeCount++;
        this._callsSinceLastCommitOrWrite = 0;
        this._readCount = 0; // reset spiral counter on write
        this._researcherWrites++;

        if (!this._firstWriteAt) {
          this._firstWriteAt = new Date(now).toISOString();
        }
      ...
      // Detect test runs in Bash commands
      if (toolName === "Bash" && typeof input.command === "string") {
        const cmd = input.command;
        if (/npm\s+test|jest|vitest|mocha|karma|ng\s+test/.test(cmd)) {
          this._testRunCount++;
        }
        // Reset explore counter for builders on non-explore bash commands
      ...
      if (this._thresholds.spiral_explorer && isRead) {
        if (this._readCount >= this._thresholds.spiral_explorer.readCountWithoutWrite) {
          ...
          this._readCount = 0; // reset to avoid repeated firing
        }
      }
      ...
      if (this._thresholds.loop) {
        ...
        if (count >= this._thresholds.loop.repeatCount) {
          violations.push(this._makeViolation("loop", {
            tool: toolName,
            repeatCount: count,
            threshold: this._thresholds.loop.repeatCount,
            inputHash: h,
          }));
          this._inputHashes.set(h, 0); // reset to avoid repeated firing
        }
      }
      ...
      if (this._thresholds.late_output && !isWrite) {
        ...
        if (this._callsSinceLastCommitOrWrite >= this._thresholds.late_output.callsWithoutCommitOrWrite) {
          ...
          this._callsSinceLastCommitOrWrite = 0; // reset
        }
      }
      ...
      if (this._thresholds.role_violation) {
        ...
        if (role === "researcher" && this._researcherWrites > this._thresholds.role_violation.researcherWriteLimit) {
          violations.push(this._makeViolation("role_violation", {
            role,
            action: "researcher writing files",
            writeCount: this._researcherWrites,
            threshold: this._thresholds.role_violation.researcherWriteLimit,
          }));
          this._researcherWrites = 0;
        }
        if (role === "builder" && this._builderExplores > this._thresholds.role_violation.builderExploreLimit) {
          violations.push(this._makeViolation("role_violation", {
            role,
            action: "builder excessive exploration",
            exploreCount: this._builderExplores,
            threshold: this._thresholds.role_violation.builderExploreLimit,
          }));
          this._builderExplores = 0;
        }
      }
